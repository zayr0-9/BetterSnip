...
function getWindowsForegroundApp(): Promise<ForegroundInfo> {
  const script = `
Add-Type @"
using System;
  ...
  return new Promise((resolve) => {
    execFile(
      ...
      (err, stdout) => {
        ...
        try {
          const parsed = JSON.parse(stdout.trim()) as Partial<ForegroundInfo>;
...
export async function getForegroundApp(): Promise<ForegroundInfo> {
...
export function buildForegroundSlug(
  info: Partial<ForegroundInfo> | null | undefined,
): string {

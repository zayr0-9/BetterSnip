...
import type { AppConfig, LmStudioSettings, QueueJob } from "../types";
...
let activeSettings: AppConfig | null = null;
const MAX_ATTEMPTS = 5;
const BACKOFF_MS = [30_000, 120_000, 300_000, 900_000, 1800_000];

export function descriptionsPath(saveDir: string): string {
  return path.join(saveDir, "descriptions.json");
}

export function queuePath(saveDir: string): string {
...
function readDescriptions(saveDir: string): DescriptionRecord {
  return readJson<DescriptionRecord>(descriptionsPath(saveDir), {});
}

function writeDescriptionRecord(
  saveDir: string,
  imageName: string,
  ...
  const existing =
    typeof data[imageName] === "object" && data[imageName] !== null
      ? data[imageName]
...
function readQueue(saveDir: string): QueueJob[] {
  const q = readJson<unknown>(queuePath(saveDir), []);
  return Array.isArray(q) ? (q as QueueJob[]) : [];
}

function writeQueue(saveDir: string, q: QueueJob[]): void {
  writeJson(queuePath(saveDir), q);
}

function cleanModelText(text: unknown): string {
  let out = String(text || "");
  ...
  return out
    ...
    .replace(
      /<\|?\/?(thought|analysis|reasoning)\|?>[\s\S]*?<\|?\/?(final|answer)\|?>/gi,
      "",
    )
    .replace(/<\|[^>]+\|>/g, "")
    .trim();
...
function extractText(json: ChatJson): string {
  const text =
...
function baseUrl(lm: Partial<LmStudioSettings>): string {
  const host = (lm.address || "localhost").replace(/^https?:\/\//, "");
  const port = lm.port || "1234";
  return `http://${host}:${port}`;
}

export async function loadModel(lm: Partial<LmStudioSettings>): Promise<void> {
  ...
  if (!res.ok) {
    ...
    if (!/already|loaded/i.test(text))
      throw new Error(`LM Studio model load HTTP ${res.status}: ${text}`);
  }
}
...
async function chatCompletion(
  ...
  if (!res.ok)
    throw new Error(`LM Studio HTTP ${res.status}: ${await res.text()}`);
  return res.json();
}
...
export function enqueueDescription(
  filePath: string,
  settings: AppConfig,
): void {
  const lm = settings.lmStudio;
  ...
  if (
    !q.some(
      (j) =>
        j.imageName === imageName && !["done", "failed"].includes(j.status),
    )
  ) {
    q.push({
      imageName,
    ...
    writeDescriptionRecord(saveDir, imageName, {
      status: "pending",
      description: "",
      error: "",
      createdAt: nowIso(),
...
export function startDescriptionQueue(settings: AppConfig): void {
  activeSettings = settings;
  if (settings?.lmStudio?.enabled) scheduleQueue(0);
}

function scheduleQueue(delay = 1000): void {
...
async function processQueue(): Promise<void> {
  ...
  try {
    ...
    const job = q.find(
      (j) =>
        ["pending", "retry"].includes(j.status) && (j.nextRunAt || 0) <= now,
    );
    if (!job) {
      const next = q
    ...
    writeQueue(saveDir, q);
    writeDescriptionRecord(saveDir, job.imageName, {
      status: "processing",
      error: "",
    });
    ...
    if (q.some((j) => ["pending", "retry"].includes(j.status)))
      scheduleQueue(1000);
  }
}
